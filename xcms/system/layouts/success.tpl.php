		...
		<section id="confirmation" class="section">
			<div class="container">
				<div class="confirmation-box-container">
					<div class="row">
						<h3 class="confirmation-title"><?php echo $_SESSION['msg'];?><?php echo $_SESSION['pacifare']['msg'];?></h3>
					</div>
					<div class="row">
						<form class="form-horizontal" role="form">
						<?if ($_SESSION['loc'] == false && $_SESSION['back'] == false): ?>
							<button type='button' class="btn btn-success" onClick="self.location = '<? echo $_SERVER['PHP_SELF']?>?action=logmein'">Weiter</button>
						<?php elseif ($_SESSION['loc'] == false && $_SESSION['back'] == true): ?>
							<button type='button' class="btn btn-success" onclick="history.back(-1)">zur&uuml;ck zum User</button>
							<button type='button' class="btn btn-success" onClick="self.location = '<? echo $_SERVER['PHP_SELF']?>'">weiter zur &Uuml;bersicht</button>
						<?php else: ?>
							<button type='button' class="btn btn-success" onClick="self.location = '<?php echo $_SERVER['PHP_SELF']?>?action=<?php echo $_SESSION['loc']?>'">weiter</button>
						<?php endif;?>
						</form>
					</div>
				</div>
